"""Data extraction workflows for the European Investment Bank (EIB).
Data is retrieved by generating URLs to page through project data via
API requests and then, after issuing the requests, parsing the details
returned by the response payloads. In a final step, those details are
used to build a URL to the project webpage and scrape that page for
...
class EibResultsMultiScrapeWorkflow(ResultsMultiScrapeWorkflow):
    """Requests pages of EIB project records from the API to
    extract project data and generate URLs to project webpages.
    """
    ...
    def scrape_results_page(self, url: str) -> Tuple[List[str], List[Dict]]:
        ...
        try:
            ...
            raise RuntimeError("Error parsing EIB projects into JSON.")
...
class EibProjectPartialScrapeWorkflow(ProjectPartialScrapeWorkflow):
    """Requests an EIB project summary page and scrapes the HTML
    for data related to project promoters and financial intermediaries.
    """
