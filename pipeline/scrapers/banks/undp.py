...
from pipeline.constants import PROJECT_PAGE_WORKFLOW, UNDP_ABBREVIATION
...
class UndpSeedUrlsWorkflow(SeedUrlsWorkflow):
    ...
    def generate_seed_urls(self) -> List[str]:
        ...
        """ 
        try:
            # Fetch list of unique projects from UNDP's public API
            r = self._data_request_client.get(
                url=self.project_list_base_url,
                timeout_in_seconds=180)
            if not r.ok:
                raise RuntimeError("The request to download projects "
                                   f"failed with a \"{r.status_code}-"
                                   f"{r.reason}\" status code and the "
                                   f"message \"{r.text}\".") from None
            
            # Read text directly into Pandas DataFrame
