...
class LocalStorageReader(IDataReader):
    ...
    def get_filepath_or_buffer(self, storage_dir_name: str, file_name: str) -> str:
        """Generates a local path for the given file.

        Args:
            storage_dir_name: The path for the file's
                immediate parent directory.

            file_name: The file name (e.g., "project.csv")

        Returns:
            The file path.
        """
        filepath = f"{settings.LOCAL_STORAGE_DIR}/{storage_dir_name}/{file_name}"
...
class CloudStorageReader(IDataReader):
    ...
    def __init__(self, cloud_project_name: str) -> None:
        ...
        Args:
            cloud_project_name: The Google Cloud
    ...
    def get_filepath_or_buffer(self, storage_dir_name: str, file_name: str) -> StringIO:
        ...
        Args:
            storage_dir_name: The name of storage bucket.

            file_name: The name of the blob to download.

        Returns:
            The downloaded blob (i.e., file).
        """
        try:
...
class IDataWriter(ABC):
    ...
    @abstractmethod
    def write_records(self, storage_location: str, records: pd.DataFrame) -> None:
        """Writes records in a Pandas DataFrame to storage.

        Args:
            storage_location: The name of, or path to, the storage location.
...
class LocalStorageWriter(IDataWriter):
    ...
    def write_records(self, storage_location: str, records: pd.DataFrame) -> None:
        ...
        Args:
            storage_location: The file path where the output file is written.

            file_name: The name of the file to write (e.g., "project.csv").
        ...
        if not segments:
            raise ValueError("Unable to write records. Must provide a valid file path.")
        file_name = segments[-1]
        parent_dir = f"{settings.EXTRACT_TEST_DIR}/{'/'.join(segments[:-1])}"
...
class CloudStorageWriter(IDataWriter):
    ...
    def __init__(self, cloud_project_name: str) -> None:
        ...
        Args:
            cloud_project_name: The Google Cloud project holding storage data.
    ...
    def write_records(self, storage_location: str, records: pd.DataFrame) -> None:
        ...
        Args:
            storage_location: The bucket to hold the file.
