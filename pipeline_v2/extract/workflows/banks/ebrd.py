"""Data extraction workflows for the European Bank for Reconstruction
and Development (EBRD). Data is retrieved by downloading a CSV file and
parsing it for both partial project details and links to project webpages.
The webpages are then scraped using both rules and LLM prompting to retrieve
information on loan amounts, associated companies, and approval dates.
"""
...
class EbrdProjectPartialDownloadWorkflow(ProjectPartialDownloadWorkflow):
    ...
    def get_projects(self) -> pd.DataFrame:
        """Retrieves all development bank projects through a direct
...
class EbrdProjectPartialScrapeWorkflow(ProjectPartialScrapeWorkflow):
    ...
    def _build_prompt(self, soup: BeautifulSoup) -> str:
        """Builds a prompt for an LLM to extract loan
        details from the raw text of webpage.
    ...
    def _prompt_ai(self, prompt: str) -> Dict | None:
        """Prompts the Gemma 3 27B model to extract loan
        details from the raw text of webpage.
        ...
        try:
            ...
            try:
                ...
                raise ValueError("Could not parse JSON from response text.")

            # Otherwise, validate JSON and return
            assert "loan_amount" in payload
