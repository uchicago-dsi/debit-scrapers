...
class TestWorkflows(ScraperFixturesMixin):
    ...
    def test_download(self, bank: str, data_request_client: DataRequestClient) -> None:
        ...
        clean_projects.to_csv(
            settings.EXTRACT_TEST_RESULT_DIR / f"{bank}_clean_projects.csv",
            index=False,
        )
    ...
    def test_partial_download(
        ...
        with open(settings.EXTRACT_TEST_RESULT_DIR / f"{bank}_urls.yaml", "w") as f:
            yaml.dump(urls, f, encoding="utf-8")
        clean_projects.to_csv(
            settings.EXTRACT_TEST_RESULT_DIR / f"{bank}_projects.csv",
            index=False,
        )
    ...
    def test_generate_seed_urls(
        ...
        with open(
            settings.EXTRACT_TEST_RESULT_DIR / f"{bank}_seed_urls.yaml", "w"
        ) as f:
            yaml.dump(urls, f, encoding="utf-8")
    ...
    def test_project_page_scrape(
        ...
        with open(
            settings.EXTRACT_TEST_RESULT_DIR / f"{bank}_project.yaml",
            "w",
            encoding="utf-8",
        ) as f:
    ...
    def test_project_partial_page_scrape(
        ...
        with open(
            settings.EXTRACT_TEST_RESULT_DIR / f"{bank}_project_partial.yaml",
            "w",
        ) as f:
    ...
    def test_result_page_multi_scrape(
        ...
        """Test of the `ResultsMultiScrapeWorkflow`.
        ...
        with open(
            settings.EXTRACT_TEST_RESULT_DIR / f"{bank}_result_page_multi_scrape.yaml",
            "w",
        ) as f:
    ...
    def test_result_page_scrape(
        ...
        """Test of the `ResultsScrapeWorkflow`.
        ...
        with open(
            settings.EXTRACT_TEST_RESULT_DIR / f"{bank}_result_page_scrape.yaml",
            "w",
        ) as f:
            yaml.dump(urls, f, encoding="utf-8")
