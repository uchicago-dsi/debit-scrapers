...
class EibResultsMultiScrapeWorkflow(ResultsMultiScrapeWorkflow):
    ...
    def _map_project_record(self, project: dict) -> dict:
        ...
        if status_date:
            ...
            approved_utc = formatted_status_date if status == "Approved" else ""
            signed_utc = (
        ...
        proposed_amt = float(proposed_amt) if proposed_amt is not None else None
        financed_amt = float(financed_amt) if financed_amt is not None else None
        amount = (
